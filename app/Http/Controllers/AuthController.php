    ...
    public function add_new_address(Request $request){
     ...
     public function update_profile(Request $request){
        ...
        //    $input = $request->all();
                ...
                $response = $response->put('https://siamtheatre.com/api/v1/user_control/profile', [
                    'firstname' => $request->first_name,
                    'lastname' => $request->last_name,
                    'gender' => $sexx,
                    'phoneNumber' => $request->phone,
                    'dateOfBirth' => $request->hbd;
                    'email' => $request->email,
                ]);
    ...
    public function add_new_api_service(Request $request){
        ...
        $qrtype = qr_code_type::where('status', 1)->get();
           ...
           // dd($bill->one_my_type);
            ...
            $message = "ชื่อบริษัท / ร้านค้า ".$request['name_company'].", ได้ทำการสมัคร ขอใช้งาน API เข้ามาใหม่";
            $lineapi = env('line_token');
